...
from embeddings import embed_texts, embed_texts_async
from qdrant_store import ensure_collection, upsert_chunks, search, search_async
from reranking import rerank_with_fallback, rerank_with_fallback_async, analyze_reranking_impact, RerankResult
...
async def ask_dataset_async(
    ...
    q_vecs = await embed_texts_async([question])
    ...
    hits = await search_async(dataset_id, version, q_vec, k_retrieval)
    ...
    if use_reranking and chunks_for_reranking:
        ...
        reranked_chunks = await rerank_with_fallback_async(
            question=question,
            chunks=chunks_for_reranking,
            top_k=k_final,
            enable_reranking=True
    ...
    if contexts:
        answer = await generate_answer_async(question, contexts, system_prompt=system_prompt)
    else:
        answer = "Не найдено релевантной информации для ответа на вопрос."
